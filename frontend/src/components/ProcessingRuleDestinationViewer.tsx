...
const ProcessingRuleDestinationViewer: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setDestinations(allDest.filter(d => d.processing_rule_id === Number(ruleId)));
  ...
  const resetForm = () => {
    setSelectedApi(''); setSelectedFtp(''); setSelectedLocal('');
    setErrors(null);
  };
  ...
  const validate = () => {
    ...
    if (chosen.length !== 1) {
      setErrors('Select exactly one destination type'); return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: any = { processing_rule_id: Number(ruleId) };
    if (selectedApi) payload.api_id = selectedApi;
    if (selectedFtp) payload.ftp_id = selectedFtp;
    if (selectedLocal) payload.local_folder_id = selectedLocal;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await fetch(`/api/processing-rules/destinations/${id}`, { method: 'DELETE' });
  ...
  if (loading) return <Container className="py-4 text-center"><Spinner /></Container>;
        ...
        <Col md={6} className="mx-auto p-4 bg-light rounded">
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3">
              ...
              <Form.Select value={selectedApi} onChange={e => { resetForm(); setSelectedApi(+e.target.value); }}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select value={selectedFtp} onChange={e => { resetForm(); setSelectedFtp(+e.target.value); }}>
                <option value="">-- Select FTP --</option>
                {ftps.map(f => <option key={f.id} value={f.id}>{f.title}</option>)}
              </Form.Select>
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Local Folder</Form.Label>
              <Form.Select value={selectedLocal} onChange={e => { resetForm(); setSelectedLocal(+e.target.value); }}>
                <option value="">-- Select Local Folder --</option>
                {locals.map(l => <option key={l.id} value={l.id}>{l.title}</option>)}
              </Form.Select>
            </Form.Group>
      ...
      <Row>
        <Col>
          ...
          <Table striped bordered hover responsive>
            <thead><tr><th>#</th><th>Type</th><th>Title</th><th>Active</th><th>Actions</th></tr></thead>
            <tbody>
              {destinations.map((d, idx) => {
                const type = d.external_api_endpoint_id ? 'API' : d.external_ftp_endpoint_id ? 'FTP' : 'Local';
                ...
                return (
                  <tr key={d.id}>
                    <td>{idx+1}</td><td>{type}</td><td>{title}</td><td className="text-center">{d.is_active ? '✓' : '✗'}</td>
