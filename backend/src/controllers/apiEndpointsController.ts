...
type ExternalAPIEndpointCreationAttributes = Optional<
  ExternalAPIEndpointAttributes,
  | "id"
  | "title"
  | "description"
  | "auth_type"
...
type ExternalAPIEndpointUpdateAttributes =
  Partial<ExternalAPIEndpointCreationAttributes>;
...
class APIEndpointsController {
  ...
  static async create(req: Request, res: Response) {
    const jsonReq: ExternalAPIEndpointCreationAttributes = req.body || {};

    const requiredFields: {
      key: keyof ExternalAPIEndpointCreationAttributes;
      name: string;
    }[] = [
    ...
    const typeValidations: {
      key: keyof ExternalAPIEndpointAttributes;
      name: string;
  ...
  static async update(req: Request, res: Response) {
    const { id } = req.params;
    const jsonReq: ExternalAPIEndpointUpdateAttributes = req.body || {};
    const numericId = parseInt(id, 10);
    ...
    for (const validation of typeValidations) {
      if (jsonReq[validation.key] !== undefined) {
        const value = jsonReq[validation.key];
        if (value === null) {
          const nullableFields: (keyof ExternalAPIEndpointAttributes)[] = [
            "title",
            "description",
            "auth_type",
            "auth_credentials",
